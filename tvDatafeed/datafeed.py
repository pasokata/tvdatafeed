...
class TvDatafeedLive(tvDatafeed.TvDatafeed):
    ...
    class _SeisesAndTrigger(dict):
        ...
        def wait(self):
            ...
            while (
                ...
                wait_time = (
                    self._trigger_dt - dt.now()
                )  # calculate the time to next expiry
        ...
        def discard(self, seis):
            ...
            if seis not in self:
                ...
                if not super().__getitem__(seis.interval.value)[
                    ...
                    if (
                        (trigger_dt := self._next_trigger_dt()) != self._trigger_dt
                    ) and (
    ...
    def _args_invalid(self, symbol, exchange):
        ...
        for item in result_list:
            if (
                item["symbol"].lower() == symbol.lower()
                and item["exchange"].lower() == exchange.lower()
            ):
                return False
    ...
    def new_seis(self, symbol, exchange, interval, timeout=-1):
        ...
        if self._main_thread is None:  # if main thread is not running then start
            self._main_thread = threading.Thread(
                name="main_loop", target=self._main_loop
            )
        ...
        while (
            ...
            with self._lock:
                for (
                    ...
                    self._sat.get_expired()
                ):  # returns a list of intervals that have expired
                    for seis in self._sat[
                        interval
                    ]:  # go through all the seises in this interval group
                        for _ in range(
                            0, RETRY_LIMIT
                        ):  # re-try maximum of RETRY_LIMIT times
                            data = super().get_hist(
                                seis.symbol,
                                seis.exchange,
                            ...
                            if data is not None:  # check that we did get any data
                                if seis.is_new_data(
                                    data
                                ):  # check that it is new data not old
                                    data = data.drop(
                                        labels=data.index[1]
                            ...
                            self._sat.quit()
                            logger.critical(
                                "Failed to retrieve new data from TradingView"
                            )
